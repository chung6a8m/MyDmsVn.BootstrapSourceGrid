...
SourceGrid/native scrollbars and the SourceGrid editor architecture remain intact. Some native editor chrome can retain OS rendering. Automatic theming applies only to exact SourceGrid default View singletons. The package targets concrete `SourceGrid.Grid`, is Windows-only, and does not provide a separate Bootstrap virtual-grid type.
