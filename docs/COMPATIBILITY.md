...
The integration product project sets `LangVersion` to `12.0` for both target frameworks. This is required because nullable reference types are enabled while the `net48` compiler default is C# 7.3, and it matches the language version proven by the pinned Bootstrap5WinFormUI baseline. Keep this setting scoped to the integration project; do not alter vendor project files.
